...
function toArray(text) {
  ...
  var regex = /\B@\[@[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]+]\(\w+\)/g;

  return (0, _stringReplaceToArray2.default)(text.replace(regex, function (match) {
    var name = match.replace(/(^.*\[|\].*$)/g, "");
    return "<span class=\"mention-name\">" + name + "</span>";
  }).replace(asciiAliasesRegex, replaceAsciiAliases).replace(aliasesRegex, replaceAliases), unicodeEmojiRegex, replaceUnicodeEmoji);
}

var parseMentionsInText = function parseMentionsInText(text) {
  var regex = /\B@\[@[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]+]\(\w+\)/g;
  var mentionText = text.replace(regex, function (match) {
    var name = match.replace(/(^.*\[|\].*$)/g, "");
    return _react2.default.createElement(
      "span",
      { "class": "mention-name" },
  ...
  return (0, _reactHtmlParser2.default)(mentionText);
};
...
function Emoji(_ref) {
  ...
  return _react2.default.createElement(
    ...
    (0, _reactHtmlParser2.default)(output)
  );
}
