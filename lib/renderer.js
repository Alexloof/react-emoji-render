...
function toArray(text) {
  ...
  var emojiText = (0, _stringReplaceToArray2.default)(text.replace(asciiAliasesRegex, replaceAsciiAliases).replace(aliasesRegex, replaceAliases), unicodeEmojiRegex, replaceUnicodeEmoji);
  return emojiText.join(" ").replace(regex, function (match) {
    var name = match.replace(/(^.*\[|\].*$)/g, "");
    return (0, _reactHtmlParser2.default)("<span class=\"mention-name\">" + name + "</span>");
  });
}
